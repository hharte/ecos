			   ...
			   unsigned char *buf, uint32_t offset, uint32_t len)
{
	...
	while(offset < end) {
		...
		if (!frag || frag->ofs > offset) {
			...
			uint32_t readlen;
			uint32_t frag_ofs = offset - frag->ofs; /* Offset within this frag from which we want to start reading */
			readlen = min(frag->size - frag_ofs, end - offset);
			D1(printk(KERN_DEBUG "Reading %d-%d from node at 0x%x\n", frag->ofs+frag_ofs, frag->ofs+frag_ofs+readlen, frag->node->raw->flash_offset & ~3));
			ret = jffs2_read_dnode(c, frag->node, buf, frag->ofs + frag_ofs - frag->node->ofs, readlen);
			D2(printk(KERN_DEBUG "node read done\n"));
			if (ret) {
				D1(printk(KERN_DEBUG"jffs2_read_inode_range error %d\n",ret));
				memset(buf, 0, frag->size - frag_ofs);
				return ret;
			}

			buf += frag->size - frag_ofs;
			offset += frag->size - frag_ofs;
			frag = frag->next;
