...
/* Peek/Poke fast mutex lock */
#define IX_QMGR_PEEKPOKE_FAST_MUTEX_TRY_LOCK(QID)\
ixOsServFastMutexTryLock(&ixQMgrAqmIfPeekPokeFastMutex[(QID)])

/* Peek/Poke fast mutex unlock */
#define IX_QMGR_PEEKPOKE_FAST_MUTEX_UNLOCK(QID)\
ixOsServFastMutexUnlock(&ixQMgrAqmIfPeekPokeFastMutex[(QID)])
...
#define IX_QMGR_PHYSICAL_AQM_BASE_ADDRESS IXP425_QMGR_BASE
...
UINT32 ixQMgrAqmIfQueUOStatRegAddr[IX_QMGR_MIN_QUEUPP_QID];
UINT32 ixQMgrAqmIfQueUflowStatBitMask[IX_QMGR_MIN_QUEUPP_QID];
UINT32 ixQMgrAqmIfQueOflowStatBitMask[IX_QMGR_MIN_QUEUPP_QID];
...
/*
 * Global for unit test purposes
 */
UINT32 aqmVirtualAddress = IX_QMGR_PHYSICAL_AQM_BASE_ADDRESS;

/* 
 * Fast mutexes, one for each queue, used to protect peek & poke functions
 */
IxFastMutex ixQMgrAqmIfPeekPokeFastMutex[IX_QMGR_MAX_NUM_QUEUES];
...
ixQMgrAqmIfInit (void)
{
     ...
     * assigned by the MMU.
     */
    ixQMgrAqmIfBaseAddressSet (aqmVirtualAddress);

    ixQMgrAqmIfRegistersReset ();

    for (i = 0; i< IX_QMGR_MAX_NUM_QUEUES; i++)
    {
	ixOsServFastMutexInit(&ixQMgrAqmIfPeekPokeFastMutex[i]);
	...
	if (i < IX_QMGR_MIN_QUEUPP_QID)
	{
	    ...
	    ixQMgrAqmIfQueUflowStatBitMask[i] = (IX_QMGR_UNDERFLOW_BIT_OFFSET + 1) <<
	    ...
	    ixQMgrAqmIfQueOflowStatBitMask[i] = (IX_QMGR_OVERFLOW_BIT_OFFSET + 1) <<
		((i & (IX_QMGR_QUEUOSTAT_NUM_QUE_PER_WORD - 1)) *
		 (BITS_PER_WORD / IX_QMGR_QUEUOSTAT_NUM_QUE_PER_WORD));
...
/*
 * Get the read & write pointers of a queue
 */
void
				 ...
				 UINT32 **writePtr)
{
    ...
    /* Read the register */
    ixQMgrAqmIfWordRead (address,&regWord);
     
    /* Extract read and write pointers */
    *readPtr = (UINT32 *)((regWord >> IX_QMGR_Q_CONFIG_RDPTR_OFFSET)&IX_QMGR_RDPTR_MASK);
    *writePtr = (UINT32 *)((regWord >> IX_QMGR_Q_CONFIG_WRPTR_OFFSET)&IX_QMGR_WRPTR_MASK);
}
			    ...
			    UINT32 **address)
{
    UINT32 readPtr;
    UINT32 baseAddress;
    UINT32 *topOfAqmSram = ((UINT32 *)aqmBaseAddress + IX_QMGR_AQM_ADDRESS_SPACE_SIZE_IN_WORDS);
    ...
    *address = (UINT32 *)(baseAddress + (readPtr * (IX_QMGR_NUM_BYTES_PER_WORD)));
	...
	case IX_QMGR_Q_ENTRY_SIZE1:
	    IX_ASSERT((*address + IX_QMGR_ENTRY1_OFFSET) < topOfAqmSram);	    
	    break;
	case IX_QMGR_Q_ENTRY_SIZE2:
	    IX_ASSERT((*address + IX_QMGR_ENTRY2_OFFSET) < topOfAqmSram);
	    break;
	case IX_QMGR_Q_ENTRY_SIZE4:
	    IX_ASSERT((*address + IX_QMGR_ENTRY4_OFFSET) < topOfAqmSram);
	    break;
	default:
		  ...
		  unsigned int *entry)
{
    ...
    if (IX_SUCCESS != IX_QMGR_PEEKPOKE_FAST_MUTEX_TRY_LOCK(qId))
    {
	return IX_FAIL;
    }
    ...
    /* Release the lock */
    IX_QMGR_PEEKPOKE_FAST_MUTEX_UNLOCK(qId);
