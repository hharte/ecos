 ...
 * @param entryHashFunction hash function used to hash entire hash node data block (for adding)
 * @param keyHashFunction hash function used to hash only the key portion of the node data block (for searching/removing)
 * @param matchFunction match function used to differentiate records with the same hash value
 ...
 * @internal
 */
void ixEthDBInitHash(HashTable *hashTable, UINT32 numBuckets, HashFunction entryHashFunction, HashFunction keyHashFunction, MatchFunction matchFunction, FreeFunction freeFunction)
{
    UINT32 bucketIndex;
    ...
    hashTable->keyHashFunction    = keyHashFunction;
    hashTable->matchFunction      = matchFunction;
    ...
    {
        ixOsServFastMutexInit(&hashTable->bucketLocks[bucketIndex]);
    }
}
 ...
 * @internal
 */
IxEthDBStatus ixEthDBAddHashEntry(HashTable *hashTable, void *entry)
{
    ...
    ixOsServFastMutexInit(&newNode->lock);
 ...
 * @internal
 */
IxEthDBStatus ixEthDBRemoveHashEntry(HashTable *hashTable, void *reference)
{
    UINT32 hashValue       = hashTable->keyHashFunction(reference);
    UINT32 bucketIndex     = hashValue % hashTable->numBuckets;
    ...
    while (node != NULL)
    {
        ...
        if (hashTable->matchFunction(reference, node->data))
        {
            /* found entry */
            if (node->next != NULL)
            {
 ...
 * @internal
 */
IxEthDBStatus ixEthDBSearchHashEntry(HashTable *hashTable, void *reference, HashNode **searchResult)
{
    UINT32 hashValue;
    HashNode *node;

    hashValue = hashTable->keyHashFunction(reference);
...
IxEthDBStatus ixEthDBIncrementHashIterator(HashTable *hashTable, HashIterator *iterator)
{
    ...
    {
        HashNode *node = hashTable->hashBuckets[iterator->bucketIndex];
